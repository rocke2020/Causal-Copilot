...
def missing_ratio_table(global_state):
    ...
    if (
        global_state.statistics.heterogeneous
        and global_state.statistics.domain_index is not None
    ):
    ...
    ratio_record_df = pd.DataFrame(
        list(ratio_record.items()), columns=["Feature", "Missingness Ratio"]
    )

    plt.figure(figsize=(4, 2))
    plt.axis("off")
    table = plt.table(
        cellText=ratio_record_df.values,
        colLabels=ratio_record_df.columns,
        loc="center",
        cellLoc="center",
    )
    ...
    print(
        f"Saving missingness ratio table to {os.path.join(save_path, 'missing_ratios_table.jpg')}"
    )
...
def drop_greater_miss_50_feature(global_state):
    ...
    ratio_greater_05 = [
        k for k, v in global_state.statistics.miss_ratio.items() if v >= 0.5
    ]
    # if global_state.user_data.drop_important_var:
    ratio_greater_05_drop = [
        element
        for element in ratio_greater_05
        if element not in global_state.user_data.important_features
    ]  # keep important features

    # Update global state
    global_state.user_data.selected_features = [
        element
        for element in global_state.user_data.selected_features
        if element not in ratio_greater_05_drop
    ]
...
def llm_select_dropped_features(global_state):
    ratio_between_05_03 = [
        k for k, v in global_state.statistics.miss_ratio.items() if 0.5 > v >= 0.3
    ]

    client = LLMClient()
    prompt = (
        f"Given the list of features of a dataset: {global_state.user_data.selected_features} \n\n,"
    ...
    llm_drop_feature = [
        element for element in ratio_between_05_03 if element not in llm_select_feature
    ]
    llm_drop_keep_important = [
        element
        for element in llm_drop_feature
        if element not in global_state.user_data.important_features
    ]  # keep important features
    ...
    if (
        global_state.statistics.domain_index is not None
        and global_state.statistics.domain_index
        not in global_state.user_data.raw_data.columns
    ):
        print(
            f"[DEBUG] domain_index '{global_state.statistics.domain_index}' not found in data columns. Setting to None."
        )
...
def drop_greater_miss_between_30_50_feature(global_state):
    ...
    if user_drop:
        global_state.user_data.selected_features = [
            element
            for element in global_state.user_data.selected_features
            if element not in user_drop
        ]
    else:
        global_state.user_data.selected_features = [
            element
            for element in global_state.user_data.selected_features
            if element not in global_state.user_data.llm_drop_features
        ]
    return global_state
    ...
    for col in data.columns:
        if col_types[col] == "Category":
            ...
            if unique_num == 1:
                ...
                global_state.user_data.selected_features = [
                    feature
                    for feature in global_state.user_data.selected_features
                    if feature != col
                ]
...
def correlation_check(global_state):
    ...
    for column in df.columns:
        ...
        if not pd.api.types.is_numeric_dtype(non_nan_data):
            ...
            df[column] = df[column].cat.codes.replace(
                -1, np.nan
            )  # Keep NaN while converting
    ...
    highly_correlated_vars = set(
        list(correlated_groups.keys())
        + [item for sublist in correlated_groups.values() for item in sublist]
    )
...
def impute_time_series(
    df: pd.DataFrame, time_index_feature: str = None
) -> pd.DataFrame:
    """
    ...
    for column in df_copy.columns:
        if column != time_index_feature and pd.api.types.is_numeric_dtype(
            df_copy[column]
        ):
            df_copy[column] = (
                pd.DataFrame(df_copy[column].values, index=time_index)
...
def series_lag_est(time_series, nlags=20, acf_threshold=0.6, pacf_top_pct=0.4):
    ...
    candidate_lags = list(
        set(acf_peaks) | set(pacf_significant_lags) | {best_aic_lag, best_bic_lag}
    )
    candidate_lags = [lag for lag in candidate_lags if lag > 0]
    if len(candidate_lags) > 0:
        final_lag = max(
            set(candidate_lags),
            key=lambda lag: (acf_values[lag] if lag in acf_peaks else 0)
...
def time_series_lag_est(df: pd.DataFrame, nlags=10, max_vars=50):
    ...
    if m > max_vars:
        ...
        logger.detail(
            f"Limiting time series lag estimation to {max_vars} out of {m} variables"
        )
    else:
    ...
    logger.detail(
        f"Computing lag estimation for {len(test_indices)} variables in parallel..."
    )
    est_lags = Parallel(n_jobs=-1)(
        delayed(series_lag_est)(df.iloc[:, i], nlags=nlags) for i in test_indices
    )
...
def data_preprocess(
    ...
    ts: bool = False,
    small_cardinality_as_cat=False,
    small_cardinality=5,
):
    ...
    for column in clean_df.columns:
        ...
        if pd.api.types.is_numeric_dtype(non_nan_data):
            if not small_cardinality_as_cat:
                column_type[column] = "Continuous"
            else:
                is_effective_integer = np.all(np.floor(non_nan_data) == non_nan_data)
                # Check if numeric
                if is_effective_integer and non_nan_data.nunique() <= small_cardinality:
    ...
    categorical_features = [
        key for key, value in column_type.items() if value == "Category"
    ]
    ...
    for column in categorical_features:
        ...
        clean_df[column] = clean_df[column].cat.codes.replace(
            -1, np.nan
        )  # Keep NaN while converting
...
def imputation(df: pd.DataFrame, column_type: dict, ts: bool = False):
    ...
    continuous_features = [
        key for key, value in column_type.items() if value == "Continuous"
    ]
...
def linearity_check(df_raw: pd.DataFrame, global_state):
    ...
    if corrected_result.sum() == 0:
        ...
        if num_nonlinear_pair >= 4:
            ...
            selected_models = [
                models[i] for i in non_linear_indices[:num_nonlinear_pair]
            ]
            selected_models.extend(
                [models[i] for i in linear_indices[: (4 - num_nonlinear_pair)]]
            )
...
def gaussian_check(df_raw, global_state):
    ...
    for i in range(pair_num):
        ...
        if corrected_result.sum() == 0:
            ...
            non_gaussain_indices = [
                i for i, result in enumerate(corrected_result) if result
            ]
            gaussian_indices = [
                i for i, result in enumerate(corrected_result) if not result
            ]
            ...
            if num_nongaussian_pair >= 4:
                ...
                selected_results = [
                    collect_result[i]
                    for i in non_gaussain_indices[:num_nongaussian_pair]
                ]
                selected_results.extend(
                    [
                        collect_result[i]
                        for i in gaussian_indices[: (4 - num_nongaussian_pair)]
                    ]
                )
...
def heterogeneity_check(
    df: pd.DataFrame, heterogeneity_indicator: str = "domain_index"
):
    """
    :param df: imputed data in Pandas DataFrame format.
...
def stationary_check(df: pd.DataFrame, max_test: int = 50, alpha: float = 0.1):
    ...
    logger.detail(
        f"Computing stationarity tests for {len(test_indices)} variables in parallel..."
    )
    ADF_pval = Parallel(n_jobs=-1)(delayed(adf_test_single)(i) for i in test_indices)

    corrected_ADF = multipletests(ADF_pval, alpha=alpha, method="bonferroni")[0]
    num_stationary = np.sum(corrected_ADF)
    stationary = (
        num_stationary >= len(test_indices) * 0.8
    )  # At least 80% of tested features must be stationary

    logger.detail(
        f"Stationarity test completed: {num_stationary}/{len(test_indices)} variables are stationary"
    )
    return stationary
...
def stat_info_collection(global_state):
    ...
    if (
        ...
        if domain_index in global_state.user_data.raw_data.columns:
            ...
            logger.warning(
                f"Domain index '{domain_index}' not found in data columns, skipping domain index handling"
            )
    ...
    if (
        ...
        and global_state.statistics.time_index is not None
    ):
        global_state.user_data.selected_features = [
            feature
            for feature in global_state.user_data.selected_features
            if feature != global_state.statistics.time_index
        ]
        data = global_state.user_data.raw_data.set_index(
            global_state.statistics.time_index
        )
    ...
    column_type, dataset_type = data_preprocess(
        clean_df=data, ts=global_state.statistics.time_series
    )
    ...
    if (
        global_state.statistics.missingness
        or global_state.statistics.missingness is None
    ):
        logger.detail("Performing data imputation...")
        imputed_data = imputation(
            df=data, column_type=column_type, ts=global_state.statistics.time_series
        )
    ...
    if (
        global_state.statistics.heterogeneous
        and global_state.statistics.domain_index
        in global_state.user_data.visual_selected_features
    ):
        global_state.user_data.visual_selected_features = [
            feature
            for feature in global_state.user_data.visual_selected_features
            if feature != global_state.statistics.domain_index
        ]
    ...
    if global_state.statistics.data_type == "Continuous":
        if global_state.statistics.linearity is None:
            ...
            global_state = linearity_check(
                df_raw=imputed_data, global_state=global_state
            )
            logger.detail(
                f"Linearity test completed: {'Linear' if global_state.statistics.linearity else 'Non-linear'}"
            )

        if global_state.statistics.gaussian_error is None:
            logger.detail("Testing Gaussian error assumptions...")
            global_state = gaussian_check(
                df_raw=imputed_data, global_state=global_state
            )
            logger.detail(
                f"Gaussian test completed: {'Gaussian' if global_state.statistics.gaussian_error else 'Non-Gaussian'}"
            )
    # Assumption checking for time-series data
        ...
        if global_state.statistics.time_lag is None:
            global_state.statistics.time_lag = time_series_lag_est(
                df=imputed_data, nlags=10, max_vars=50
            )
            logger.detail(f"Time lag estimated: {global_state.statistics.time_lag}")
        # check linearity
        global_state = linearity_check_ts(
            df_raw=imputed_data, global_state=global_state
        )
        # check gaussianity
        global_state = gaussian_check(df_raw=imputed_data, global_state=global_state)
        # check stationarity
        global_state.statistics.stationary = stationary_check(
            df=imputed_data,
            max_test=global_state.statistics.num_test,
            alpha=global_state.statistics.alpha,
        )
        logger.detail(
            f"Time-series analysis completed: {'Stationary' if global_state.statistics.stationary else 'Non-stationary'}"
        )
    else:
...
def missing_ratio_check(df: pd.DataFrame, nan_indicator):
    ...
    missing_ratio_dict = {
        "high": [k for k, v in ratio_record.items() if v >= 0.5],  # ratio > 0.5
        "moderate": [
            k for k, v in ratio_record.items() if 0.5 > v >= 0.3
        ],  # 0.5 > ratio >= 0.3
