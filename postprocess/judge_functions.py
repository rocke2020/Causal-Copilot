...
def bootstrap_probability(boot_result, algorithm):
    ...
    for i in range(m):
        for j in range(m):
            if i == j:
                ...
                # j o-> i
                half_certain_edges_prob[i, j] = np.mean(
                    (elements_ij == 4) & (elements_ji == 0)
                )
                # j o- i
                half_uncertain_edges_prob[i, j] = np.mean(
                    (elements_ij == 5) & (elements_ji == 0)
                )
                # i o-o j
    ...
    edges_prob = np.stack(
        (
            certain_edges_prob,
            uncertain_edges_prob,
            bi_edges_prob,
            half_certain_edges_prob,
            half_uncertain_edges_prob,
            none_edges_prob,
            none_exist_prob,
        ),
        axis=0,
...
def bootstrap(
    data, full_graph, algorithm, hyperparameters, boot_num, ts, parallel, progress=None
):
    """
    :param data: Given Tabular Data in Pandas DataFrame format
    ...
    for i in range(m):
        for j in range(m):
            if i == j:
                ...
                cond3 = half_certain_edges_raw and (
                    edges_prob[3, i, j] < 0.05
                )  # j o-> i
                cond4 = half_uncertain_edges_raw and (
                    edges_prob[4, i, j] < 0.05
                )  # j o- i
...
def bootstrap_recommend(raw_graph, boot_edges_prob):
    ...
    middle_prob_edges_direct = list(
        zip(middle_prob_idx_direct[0], middle_prob_idx_direct[1])
    )

    undirect_prob_mat = boot_edges_prob["uncertain_edges"]
    high_prob_idx_undirect = np.where(undirect_prob_mat >= 0.9)
    high_prob_edges_undirect = list(
        zip(high_prob_idx_undirect[0], high_prob_idx_undirect[1])
    )
    middle_prob_idx_undirect = np.where(
        (undirect_prob_mat < 0.9) & (undirect_prob_mat > 0.1)
    )
    middle_prob_edges_undirect = list(
        zip(middle_prob_idx_undirect[0], middle_prob_idx_undirect[1])
    )

    middle_prob_edges = list(
        set(
            low_prob_edges_direct
            + high_prob_edges_undirect
            + middle_prob_edges_direct
            + middle_prob_edges_undirect
        )
    )
...
def get_json(args, prompt):
    ...
    response = client.chat_completion(
        prompt=prompt, system_prompt="You are a helpful assistant.", json_response=True
    )
    return response
...
def call_llm_new(args, prompt, prompt_type):
    ...
    response = client.chat_completion(prompt=prompt, system_prompt=system_prompt)
    ...
    for line in lines:
        try:
            ...
            llm_answer[pair] = {"result": result, "explanation": explanation}
...
def llm_evaluation_new(
    ...
    combined_uncertain_edges = list(
        set(tuple(sorted((i, j))) for (i, j) in combined_uncertain_edges)
    )
    ...
    def check_node_relationship(main_node):
        ...
        relation_text_dict, relation_text = edges_to_relationship(
            data, edges_dict, boot_edges_prob
        )
        try:
            directed_exist_texts_mainnode = ", ".join(
                [
                    text
                    for text in relation_text_dict["certain_edges"]
                    if main_node in text
                ]
            )
        except:
            directed_exist_texts_mainnode = "None"
        try:
            undirected_exist_texts_mainnode = ", ".join(
                [
                    text
                    for text in relation_text_dict["uncertain_edges"]
                    if main_node in text
                ]
            )
        except:
        ...
        if "markov_blanket" in prompt_type:
            ...
            tuples_with_mainnode = [
                t for t in certain_edges + uncertain_edges if main_node in t
            ]
            related_nodes = [
                item for t in tuples_with_mainnode for item in t if item != main_node
            ]
            for node in related_nodes:
                if "certain_edges" in relation_text_dict:
                    directed_exist_texts_related = ", ".join(
                        [
                            text
                            for text in relation_text_dict["certain_edges"]
                            if node in text
                ...
                if "uncertain_edges" in relation_text_dict:
                    undirected_exist_texts_related = ", ".join(
                        [
                            text
                            for text in relation_text_dict["uncertain_edges"]
                            if node in text
        ...
        if vote_num == 1:
            ...
            for pair_i in merged_dict.keys():
                result_list = [
                    single_vote["result"] for single_vote in merged_dict[pair_i]
                ]
                # print('result_list',result_list)
                explanation_list = [
                    single_vote["explanation"] for single_vote in merged_dict[pair_i]
                ]
                majority_result = Counter(result_list).most_common(1)[0][0]
                majority_explanation = explanation_list[
                    result_list.index(majority_result)
                ]
                llm_answer[pair_i] = {
    ...
    for idx_1 in range(len(data.columns) - 1):
        for idx_2 in range(idx_1 + 1, len(data.columns)):
            ...
            if p_value > 0.05 and revised_graph[idx_1, idx_2] != 0:
                logger.debug(
                    f"KCI test result ({data.columns[idx_1]}, {data.columns[idx_2]}): {p_value}",
                    "KCI",
                )
        ...
        for edges in edges_list:
            if boot_edges_prob is not None:
                try:
                    ...
                    try:
                        idx_j = data.columns.str.lower().get_loc(
                            edges[0].lower().replace("_", " ")
                        )
                        idx_i = data.columns.str.lower().get_loc(
                            edges[1].lower().replace("_", " ")
                        )
                    except:
                        continue
                prob = boot_edges_prob[edge_type][idx_i, idx_j]
                result_dict[edge_type].append(
                    f"{edges[0]} {relation_dict[edge_type]} {edges[1]} with bootstrap probability {prob}"
                )
            else:
                result_dict[edge_type].append(
                    f"{edges[0]} {relation_dict[edge_type]} {edges[1]}"
                )
...
def check_cycle(args, data, graph, knowledge_docs):
    ...
    with open("causal_discovery/context/remove_cycles.txt", "r", encoding="utf-8") as f:
        context = f.read()
    # Output the result
    if acyclic:
