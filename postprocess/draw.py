    ...
    **attrs,
    ...
    if circle_edges is not None:
        for sib1, sib2 in circle_edges:
            ...
            if (
                f"{sib1}-{sib2}" in found_circle_sibs
                or f"{sib2}-{sib1}" in found_circle_sibs
            ):
                continue
...
def draw(
    ...
    **attrs,
    ...
    if data_type == "time-series":
        directed_color = (
            "#000080"  # Keep as dark blue for now (can be changed if needed)
        )
    else:
        directed_color = "#000080"
    # add the nodes that in the G but not in the PAG
    logger.info(f"{full_node_names = }")
