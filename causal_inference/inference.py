...
from causal_inference.DML.hte_filter import HTE_Filter as DML_HTE_Filter
from causal_inference.DML.hte_params import HTE_Param_Selector as DML_HTE_Param_Selector
from causal_inference.DML.hte_program import HTE_Programming as DML_HTE_Programming
from causal_inference.DRL.hte_filter import HTE_Filter as DRL_HTE_Filter
from causal_inference.DRL.hte_params import HTE_Param_Selector as DRL_HTE_Param_Selector
from causal_inference.DRL.hte_program import HTE_Programming as DRL_HTE_Programming
from causal_inference.IV.hte_filter import HTE_Filter as IV_HTE_Filter
from causal_inference.IV.hte_params import HTE_Param_Selector as IV_HTE_Param_Selector
from causal_inference.IV.hte_program import HTE_Programming as IV_HTE_Programming
from causal_inference.MetaLearners.hte_filter import HTE_Filter as MetaLearners_HTE_Filter
from causal_inference.MetaLearners.hte_params import HTE_Param_Selector as MetaLearners_HTE_Param_Selector
from causal_inference.MetaLearners.hte_program import HTE_Programming as MetaLearners_HTE_Programming
from causal_inference.help_functions import *
from causal_inference.analysis import *
...
class Analysis(object):
    def __init__(self, global_state, args):
        ...
        logger.debug(f"Graph structure: {type(self.graph)}", "Inference")
        ...
        logger.debug(f"Graph edges: {len(self.G.edges)}", "Inference")
    ...
    def _print_data_disclaimer(self):
        ...
        if object_cols:
            logger.status("Categorical columns", len(object_cols))
        if continuous_cols:
            logger.status("Continuous columns", len(continuous_cols))
        if discrete_cols:
            logger.status("Discrete columns", len(discrete_cols))
        logger.debug("Column type analysis completed", "DataAnalysis")

    def feature_importance(self, target_node, linearity, visualize=True):
        logger.process("Analyzing feature importance")
    ...
    def _propensity_score_matching(self, confounders, treatment, outcome):
        ...
        logger.status("Treatment variable", treatment)
        logger.debug(f"Treatment unique values: {self.data[treatment].nunique()}", "Treatment")
        ...
        if len(self.data[treatment].unique()) > 2:
            ...
            logger.process(f"Treatment variable binarized (threshold: {threshold:.3f})")

        # Ensure the treatment variable is a 1-dimensional array
        treatment_values = self.data[treatment].values
        logger.debug(f"Treatment values shape: {treatment_values.shape}", "Treatment")
        ...
        logger.debug(f"Confounders shape: {confounders.shape}", "PSM")
    ...
    def _check_balance(self, data, matched_data, treatment, outcome, confounders, cont_confounders, title):
        ...
        if not valid_cont_confounders:
            logger.warning("No valid continuous confounders found in matched data")
    ...
    def estimate_causal_effect_matching(self, treatment, outcome, confounders, X_col, treat, control, cont_confounders, method="propensity_score", visualize=True):
        ...
        """
        logger.section(f"Causal Effect Estimation - {method}")
        logger.status("Treatment", treatment)
        logger.status("Outcome", outcome)
        ...
        logger.success(f"Average Treatment Effect ({method}): {ate:.4f}")
    ...
    def attribute_anomalies(self, target_node, anomaly_samples, confidence_level=0.95):
        ...
        """
        logger.section(f"Anomaly Attribution Analysis - {target_node}")
        ...
        if not nx.is_directed_acyclic_graph(G):
            logger.error("Cycle removal failed - graph is still cyclic")
        ...
        if not ancestors:
            logger.error(f"Target node {target_node} has no ancestors - cannot calculate anomaly attribution")
            return None, []
        ...
        if len(selected_columns) > max_nodes:
            logger.warning(f"More than {max_nodes} nodes selected, reducing selection")
        ...
        logger.success(f"Selected {len(selected_columns)} columns for analysis")
        ...
        try:
            ...
            logger.error(f"KeyError: {e} - Mismatch between graph nodes and DataFrame columns")
            raise

        # Variance Plot for Target Node
        if self.data[target_node].isna().all():
            logger.warning(f"No data available for {target_node}, skipping variance plot")
        else:
            ...
            logger.save("Variance plot saved", var_plot_path)

        # Causal Graph with Arrow Strengths
        if len(list(self.G.predecessors(target_node))) == 0:
            logger.error(f"Target node {target_node} has no ancestors - cannot compute anomaly attribution")
            return None, []
        ...
        logger.save("Causal graph saved", causal_graph_path)
        ...
        if df.empty:
            logger.warning("No anomaly attribution results found, skipping plot generation")
        else:
            ...
            logger.save("Attribution scores plot saved", attribution_path)
    ...
    def attribute_distributional_changes(self, target_node, data_old, data_new, method='distribution_change', confidence_level=0.95):
        ...
        """
        logger.section(f"Distributional Change Attribution - {target_node}")
        logger.status("Method", method)
        ...
        if not ancestors:
            logger.error(f"Target node {target_node} has no ancestors - cannot calculate distributional change attribution")
            return None, []
        ...
        try:
            ...
            logger.error(f"Error assigning causal mechanisms: {e}")
            missing_nodes = [node for node in self.causal_model.graph.nodes if node not in self.causal_model.causal_mechanism]
            logger.warning(f"Nodes missing causal mechanisms: {missing_nodes}")
            raise
        ...
        try:
            ...
            logger.error(f"Error during distribution change computation: {e}")
            raise
        ...
        plt.show()
        plt.close()
        logger.save("Bar plot saved", bar_plot_path)
        ...
        plt.show()
        plt.close()
        logger.save("Violin plot saved", violin_plot_path)
    ...
    #         logger.info("Categorical string treatment detected")
    ...
    #         logger.info(f"Binary treatment detected: {sorted_vals}")
    #         if T0 is None or T1 is None:
    ...
    def prepare_treatment_column(
        ...
        if discretize:
            logger.info(f"Discretizing treatment '{treatment_col}' into quantile bins")
            try:
                # 3 bins: low, medium, high
                treatment_binned = pd.qcut(treatment, q=3, labels=[0, 1, 2])
            except ValueError as e:
                logger.warning(f"Could not discretize '{treatment_col}': {e}")
        ...
        if pd.api.types.is_numeric_dtype(treatment) and len(unique_vals) <= 10:
            logger.info(f"Low-cardinality numeric treatment detected: {unique_vals}")
            if T0 is None or T1 is None:
                T0, T1 = unique_vals[0], unique_vals[1]
            return treatment, T0, T1
        ...
        if model_name == 'CausalForestDML':
            logger.warning("CausalForestDML doesn't support continuous treatment, switching to LinearDML")
            self.global_state.inference.hte_algo_json['name'] = 'LinearDML'

        if T0 is None or T1 is None:
            T0 = treatment.quantile(0.1)
            T1 = treatment.quantile(0.9)
            logger.info(f"Using 10th and 90th percentiles as T0/T1: {T0:.3f}, {T1:.3f}")
    ...
    def estimate_effect_dml(self, outcome, treatment, T0, T1, X_col, W_col, query):
        ...
        if model_name == 'CausalForestDML':
            ...
            logger.info(f"Overriding T0/T1 for CausalForestDML: T0={T0}, T1={T1}")
            programmer.T0 = T0
            programmer.T1 = T1
            if programmer.model:
        ...
        logger.status("Treatment levels", f"T0={T0}, T1={T1}")
        logger.debug(f"Treatment unique values: {self.data[treatment].nunique()}", "Treatment")
    ...
    def estimate_effect_drl(self, outcome, treatment, T0, T1, X_col, W_col, query):
        ...
        if T0 not in treatment_values or T1 not in treatment_values:
            T0, T1 = treatment_values[0], treatment_values[-1]
            logger.info(f"Overriding T0/T1 to valid values: T0={T0}, T1={T1}")
            programmer.T0 = T0
            programmer.T1 = T1
            if programmer.model:
                programmer.model.T0 = T0
    ...
    def estimate_effect_iv(self, outcome, treatment, instrument_variable, T0, T1, X_col, W_col, query):
        ...
        logger.success(f"Saving HTE results to: {self.global_state.user_data.output_graph_dir}/hte.csv")
    ...
    def estimate_effect_linear(self, 
        ...
        """
        logger.section(f"Causal Effect Estimation - Linear Regression")
        logger.status("Treatment", treatment)
        logger.status("Outcome", outcome)
        logger.status("Method", "backdoor.linear_regression")
        
        logger.process("Creating Causal Model...")
        logger.debug(f"DOT graph: {self.dot_graph}", "CausalModel")
        model = CausalModel(
            data=self.data,
            treatment=treatment,
            outcome=outcome,
        ...
        logger.success("Causal estimation completed")
        logger.debug(f"Causal estimate: {causal_estimate}", "DoWhy")
        # Significance Test
        ...
        logger.status("Significance test p-value", f"{p_value:.6f}")
    ...
    def counterfactual_estimation(self, treatment_name, response_name, observed_val = None, intervened_treatment = None):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        logger.save("Counterfactual estimation plot saved", os.path.join(path, 'counterfactual_est_fig.jpg'))
    ...
    #                             shift_intervention_val=None):
        ...
        logger.save("Shift intervention comparison plot saved", os.path.join(path, 'shift_intervention.jpg'))
        plt.savefig(os.path.join(path, 'shift_intervention.jpg'))
        # figs = [os.path.join(path, 'shift_intervention.jpg')]

        # Save dataset
        # logger.save("Simulated dataset saved", os.path.join(path, 'simulated_atomic_intervention_org.csv'))
        # atomic_samples_org.to_csv(os.path.join(path, 'simulated_atomic_intervention_org.csv'), index=False)
        #
        # logger.save("Simulated dataset saved", os.path.join(path, 'simulated_atomic_intervention_new.csv'))
        # atomic_samples_new.to_csv(os.path.join(path, 'simulated_atomic_intervention_new.csv'), index=False)

        logger.save("Simulated dataset saved", os.path.join(path, 'simulated_shift_intervention.csv'))
        ...
        logger.save("Box-plot comparison saved", os.path.join(path, 'shift_intervention_boxplot.jpg'))
        ...
        plt.ylabel(response_name)
        logger.save("Violin plot comparison saved", os.path.join(path, 'shift_intervention_violinplot.jpg'))
    ...
    def sensitivity_analysis(self, target_node, model, estimand, estimate, treatment, outcome):
        ...
        logger.debug(f"Model common causes: {model.get_common_causes()}", "Sensitivity")
        ...
        logger.debug(f"Refutation results: {refute}", "Sensitivity")
...
if __name__ == '__main__':
    ...
    def parse_args():
        ...
        parser.add_argument(
            '--output-report-dir',
            type=str,
            default='causal_inference/test_result',
