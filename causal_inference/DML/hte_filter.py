...
class HTE_Filter(object):
    ...
    def load_algo_context(self):
        return open(f"causal_inference/DML/context/hte_algo.txt", "r").read()
    
    def load_select_prompt(self):
        return open(f"causal_inference/DML/context/hte_select_prompt.txt", "r").read()
    ...
    def forward(self, global_state, query):
        ...
        response = self.client.chat_completion(
            prompt=prompt,
            system_prompt="You are a helpful assistant for DML hte_filter.",
            json_response=True
        )
