...
class AcceleratedLiNGAM:
    ...
    def __init__(self):
        ...
        fit_path = 'causal_discovery/context/benchmarking/acceleration_fit.json'
        if os.path.exists(fit_path):
            with open(fit_path, 'r') as f:
                fit_data = json.load(f)
                if 'accelerated_lingam' in fit_data:
                    # Load existing coefficients
                    coef_data = fit_data['accelerated_lingam']
                    self.gpu_model = LinearRegression(fit_intercept=True)
                    self.gpu_model.coef_ = np.array(coef_data['coefficients'])
                    self.gpu_model.intercept_ = coef_data['intercept']
                    
                    # Print loaded coefficients
                    print("Loaded Model Coefficients:")
                    print("\nGPU Model:")
                    for name, coef in zip(self.feature_names, self.gpu_model.coef_):
                        print(f"{name}: {coef:.2e}")
                    print(f"intercept: {self.gpu_model.intercept_:.2e}")
                    
                    return
        # If no saved coefficients, fit model
        data = json.load(open('causal_discovery/context/benchmarking/acceleration.json', 'r'))['accelerated_lingam']
    ...
    def _fit_model(self, data):
        ...
        self.gpu_model = LinearRegression(fit_intercept=True)
        self.gpu_model.fit(X_features, df['time_cost'])
        ...
        fit_data['accelerated_lingam'] = {
            'coefficients': self.gpu_model.coef_.tolist(),
            'intercept': float(self.gpu_model.intercept_)
        }
    ...
    def predict_runtime(self, n_variables, n_samples):
        ...
        return max(self.gpu_model.predict(features)[0], 0.058)
    ...
    def explain_complexity(self, n_variables, n_samples):
        ...
        gpu_time = self.predict_runtime(n_variables, n_samples)
        print(f"Predicted GPU time: {gpu_time:.3f} seconds")

if __name__ == "__main__":
    predictor = AcceleratedLiNGAM()
