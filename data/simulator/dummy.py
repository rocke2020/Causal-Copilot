...
class DataSimulator:
    ...
    def generate_single_domain_data(self, n_samples: int, noise_scale: float, noise_type: str, 
        ...
        logger.debug(f"Data generation: {function_type}, {noise_type}, scale={noise_scale}", "Simulator")
        ...
        if function_type != 'linear':
            logger.debug("Non-linear function requires gaussian noise", "Simulator")
    ...
    def generate_multi_domain_data(self, n_samples: int, noise_scale: float, noise_type: str, 
        ...
        logger.debug(f"Domain affects {len(affected_nodes)} variables", "Simulator")
        ...
        logger.debug(f"Base correlation matrix: {base_correlation_matrix.shape}", "Simulator")
        ...
        if high_corr_pairs_base:
            logger.debug(f"Found {len(high_corr_pairs_base)} highly correlated pairs (base)", "Simulator")
        else:
            logger.debug(f"No high correlations found (base)", "Simulator")
        ...
        # Log correlation information after domain effects
        logger.debug(f"Final correlation matrix: {correlation_matrix.shape}", "Simulator")
        
        if high_corr_pairs:
            logger.debug(f"Found {len(high_corr_pairs)} highly correlated pairs (final)", "Simulator")
        else:
            logger.debug(f"No high correlations found (final)", "Simulator")
    ...
    def save_simulation(self, output_dir: str = 'simulated_data', prefix: str = 'base') -> None:
        ...
        logger.success(f"Data saved: {os.path.basename(data_filename)}")
        ...
        logger.success(f"Graph saved: {os.path.basename(graph_filename)}")
        ...
        logger.success(f"Config saved: {os.path.basename(config_filename)}")
...
def test_mixed_data_generation():
    ...
    logger.success("Test dataset generated with mixed specifications")
    return graph, data
...
class TimeSeriesSimulator:
    ...
    def save_simulation(self, output_dir: str = 'simulated_time_series', prefix: str = 'ts'):
        ...
        logger.success(f"Time series data saved: {os.path.basename(data_filename)}")
        logger.success(f"Summary matrix saved: {os.path.basename(summary_filename)}")
        logger.success(f"Lagged matrix saved: {os.path.basename(lagged_filename)}")
        logger.success(f"Config saved: {os.path.basename(config_filename)}")
...
def test_time_series_generation():
    ...
    logger.success(f"Generated time series data: {data.shape}")
    logger.status("Summary matrix shape", ts_simulator.ground_truth['summary_adjacency'].shape)
    logger.status("Lagged matrix shape", ts_simulator.ground_truth['lagged_adjacency'].shape)
...
if __name__ == "__main__":
    ...
    logger.debug(f"Graph shape: {graph.shape if hasattr(graph, 'shape') else type(graph)}", "Test")
    logger.debug(f"Data shape: {data.shape if hasattr(data, 'shape') else type(data)}", "Test")