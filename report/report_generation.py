    ...
    def __init__(self, global_state, args):
        ...
        if global_state.results.report_selected_index is not None:
            ...
            for algo in global_state.logging.global_state_logging:
                with open(
                    f"{global_state.user_data.output_graph_dir}/{algo}_global_state.pkl",
                    "rb",
                ) as f:
                    self.global_state_list.append(pickle.load(f))
            global_state = self.global_state_list[
                global_state.results.report_selected_index
            ]
        else:
...
"""
        ...
        if result_parsed != {}:
            ...
            for pair in result_parsed.keys():
                ...
                if (
                    pair[0].lower() in variables.str.lower()
                    and pair[1].lower() in variables.str.lower()
                ):
        ...
        _ = my_visual.plot_pdag(
            zero_matrix,
            "potential_relation.pdf",
            pos=self.global_state.results.raw_pos,
            relation=True,
        )
    ...
    def data_prop_prompt(self):
        ...
        time_series = "True" if self.statistics.data_type == "Time-series" else "False"
        ...
        \\begin{{tabular}}{{rrrrrrr}}
            ...
            \midrule
            {shape}   & {data_type} & {missingness} & {linearity} & {gaussian_error} & {time_series} & {heterogeneous} \\\\
            \\bottomrule
        \end{{tabular}}
        """
        return prop_table
    ...
    def ts_eda_prompt(self):
        ...
        corr_summary_text = granger_causality_to_latex(
            ts_eda_corr_summary["potential_granger_causality"]
        )
...
{corr_summary_text}
        ...
        diagnostics_summary_graph = glob.glob(
            f"{self.visual_dir}/eda_ts_diagnostics_*"
        )[:2]
...
\end{{figure}}
    ...
    def graph_effect_prompts(self, is_initial=True):
        ...
        if graph.sum() == 0:
            ...
            variables = self.data.columns
            edges_dict = convert_to_edges(self.algo, variables, graph)
            relation_text_dict, relation_text = edges_to_relationship(
                self.data, edges_dict
            )
    ...
    def confidence_graph_prompts(self):
        ...
        if self.bootstrap_probability is not None:
            bootstrap_dict = {
                k: v
                for k, v in self.bootstrap_probability.items()
                if v is not None and sum(v.flatten()) > 0
            }
            zero_graphs = [
                k
                for k, v in self.bootstrap_probability.items()
            ...
            if len(zero_graphs) < len(bootstrap_dict):
                ...
                for key in bootstrap_dict.keys():
                    ...
                    if os.path.exists(graph_path):
                        ...
                        logger.warning(
                            f"Confidence heatmap file not found: {graph_path}", "Report"
                        )
                ...
                if valid_images_found:
                    ...
                    graph_text = ""
                    logger.warning(
                        "No confidence heatmap images found - skipping heatmap section in report",
                        "Report",
                    )
                ### Generate text illustration
                text_map = {
                    "certain_edges": rf"directed edge ($\rightarrow$)",  # (->)
                    "uncertain_edges": "undirected edge ($-$)",  # (-)
                ...
                graph_text += (
                    "The heatmap of "
                    + ", ".join(zero_graphs)
                    + " is not shown because probabilities of all edges are 0. "
                )
            else:
                return ""
        else:
    ...
    def confidence_analysis_prompts(self):
        if self.bootstrap_probability is not None:
            ...
            relation_text_dict, relation_text = edges_to_relationship(
                self.data, edges_dict, self.bootstrap_probability
            )
    ...
    def comparision_prompt(self):
        if len(self.global_state_list) < 2:
            ...
            """
            algos = [
                state.algorithm.selected_algorithm for state in self.global_state_list
            ]
...
"""
            for state in self.global_state_list:
                ...
                relation_text_dict, relation_text = edges_to_relationship(
                    self.data, state.results.raw_edges
                )
...
"""
        ...
        with open("test/abstract_prompt.txt", "w", encoding="utf-8") as f:
            f.write(prompt)
        with open("test/abstract_response.txt", "w", encoding="utf-8") as f:
            f.write(str(response))
        response_doc = str(response)
        response_doc = response_doc.replace("_", r"\_")
        return response_doc
    ...
    def conclusion_prompt(self):
        prompt = f"""
根据下列信息，帮我写1-2段的分析报告的中文的结论。你应该涵盖：1. 分析了什么数据（在标题和介绍中找到）2. 用了什么方法（在发现过程找到）3. 得到了什么结果，这一点很重要（在图结果分析和图修正过程中找到）4. 我们的贡献是什么，这一点很重要（自己总结）。只包含你的文本内容在回答中。不要包含任何其他东西，比如标题，等等。
标题: {self.title} 
介绍: {self.intro_info} 
发现过程: {self.discover_process}
图结果分析: {self.graph_prompt}
图修正过程: {self.revise_process} 
Reliability Analysis: {self.reliability_prompt}
Comparison of different algorithms: {self.result_comparison} 
Revised Edges: {self.global_state.results.revised_edges}
Graph results: {self.graph_effect_prompts(is_initial=False)}

请编写中文的结论，回答中只包含文本。不要包含任何标题或额外格式。
            """
        response = self.client.chat_completion(
            prompt=prompt,
            system_prompt="You are an expert in the causal discovery field and helpful assistant.",
        ...
        with open("test/conclusion_prompt.txt", "w", encoding="utf-8") as f:
            f.write(prompt)
        with open("test/conclusion_response.txt", "w", encoding="utf-8") as f:
            f.write(str(response))
        response_doc = str(response)
        response_doc = response_doc.replace("_", r"\_")
        return response_doc
    ...
    def generation(self, debug=False):
        ...
        # Data info
        df = self.data.copy()
        # TODO [dqc] delete columns here for data preview?
        ...
        # Intro info
        self.title, dataset_name = self.get_title()
        ...
        # Graph effect info
        self.graph_prompt = bold_conversion(
            self.global_state.logging.graph_conversion["initial_graph_analysis"]
        )
        ...
        self.result_comparison_graph_text, self.result_comparison = (
            self.comparision_prompt()
        )
        ...
        if self.inference_global_state.inference.task_index != -1:
            logger.debug(
                f"Inference task index: {self.inference_global_state.inference.task_index}",
                "Report",
            )
            self.inf_report = ""
            for task_index in range(
                self.inference_global_state.inference.task_index + 1
            ):
                logger.debug(f"Processing task index: {task_index}", "Report")
                self.inf_report += (
                    f"\section{{Causal Inference Results for Task {task_index + 1}}}\n"
                )
                inf_report_generator = Inference_Report_generation(
                    self.inference_global_state, self.args, task_index
                )
        ...
        if self.data_mode == "simulated":
            prompt_template = load_context(
                "report/context/template_simulated_notruth.tex"
            )
        else:
        ...
        for i in range(num_error_corrections):
            ...
            if check_output:
                ...
                ** YOU SHOULD **
                ...
                """,
                    messages=[{"role": "user", "content": tex_content}],
                )
                output = response
                with open(
                    f"{save_path}/report_revised.tex", "w", encoding="utf-8"
                ) as file:
                    file.write(output)
            else:
                break
...
def parse_args():
    ...
    parser.add_argument(
        "--debug", action="store_true", default=True, help="Enable debugging mode"
    )
    ...
    parser.add_argument("--demo_mode", type=bool, default=False, help="Demo mode")
